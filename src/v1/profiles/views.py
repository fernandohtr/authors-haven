...
from v1.profiles.exceptions import CantFollowYourself
from v1.profiles.models import Profile
from v1.profiles.pagination import ProfilePagination
from v1.profiles.renderers import ProfileJSONRenderer, ProfilesJSONRenderer
from v1.profiles.serializers import FollowingSerializer, ProfileSerializer, UpdateProfileSerializer
...
class FollowingListView(APIView):
    def get(self, request, user_id, format=None):  # noqa
        try:
...
class FollowAPIView(APIView):
    def post(self, request, user_id, format=None):  # noqa
        try:
...
class UnfollowAPIView(APIView):
    def post(self, request, user_id, *args, **kwargs):  # noqa
        user_profile = request.user.profile
        profile = Profile.objects.get(user__id=user_id)
        ...
        if not user_profile.check_following(profile):
            formatted_response = {
                "status_code": status.HTTP_400_BAD_REQUEST,
                "message": (
                    f"You can't unfollow {profile.user.first_name} {profile.user.last_name}, "
                    "since you were not following then in the first place"
                ),
            }
            return Response(
                formatted_response,
